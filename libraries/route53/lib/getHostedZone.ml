    ...
    Uri.add_query_params
      (Uri.of_string
         ((Aws.Util.of_option_exn (Endpoints.url_of service region)) ^
            ("/2013-04-01/hostedzone/" ^ req.GetHostedZoneRequest.id)))
      (Util.drop_empty
         (Uri.query_of_encoded
            (Query.render (GetHostedZoneRequest.to_query req)))) in
  (`GET, uri, (Headers.render (GetHostedZoneRequest.to_headers req)), "")
let of_http body =
  try
    ...
    let resp =
      ...
      | `El (_, xs) -> Some xs
      | _ ->
          raise (Failure "Could not find well formed GetHostedZoneResponse.") in
    try
