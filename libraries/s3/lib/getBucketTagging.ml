    ...
    Uri.add_query_params
      (Uri.of_string
         ((Aws.Util.of_option_exn (Endpoints.url_of service region)) ^
            (("/" ^ req.GetBucketTaggingRequest.bucket) ^ "?tagging")))
      (Util.drop_empty
         (Uri.query_of_encoded
            (Query.render (GetBucketTaggingRequest.to_query req)))) in
  (`GET, uri, (Headers.render (GetBucketTaggingRequest.to_headers req)), "")
let of_http body =
  try
    ...
    let resp =
      ...
      | _ ->
          raise
            (Failure "Could not find well formed GetBucketTaggingOutput.") in
    try
