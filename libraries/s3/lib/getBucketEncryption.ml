...
let of_http headers
        ...
        try
          ...
          let resp =
            ...
            | _ ->
                raise
                  (Failure
                     "Could not find well formed ServerSideEncryptionConfiguration.") in
          try
            let open Error in
              Util.or_error
                ...
                (BadResponse
                   {
                     body;
                     message =
                       "Could not find well formed ServerSideEncryptionConfiguration."
                   })
          with
                  ...
                  (BadResponse
                     {
                       body;
                       message =
                         ("Error parsing ServerSideEncryptionConfiguration - missing field in body or children: "
                            ^ msg)
                     })
        with
        | Failure msg ->
  ...
  with
  | `Ok payload -> `Ok (GetBucketEncryptionOutput.of_headers headers payload)
  | `Error err -> `Error err
