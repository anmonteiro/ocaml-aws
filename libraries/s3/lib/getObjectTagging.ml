    ...
    Uri.add_query_params
      (Uri.of_string
         ((Aws.Util.of_option_exn (Endpoints.url_of service region)) ^
            (((("/" ^ req.GetObjectTaggingRequest.bucket) ^ "/") ^
                req.GetObjectTaggingRequest.key)
               ^ "?tagging")))
      (Util.drop_empty
         (Uri.query_of_encoded
            (Query.render (GetObjectTaggingRequest.to_query req)))) in
  (`GET, uri, (Headers.render (GetObjectTaggingRequest.to_headers req)), "")
let of_http headers
  ...
  try
    ...
    let resp =
      ...
      | _ ->
          raise
            (Failure "Could not find well formed GetObjectTaggingOutput.") in
    try
      let open Error in
