    ...
    Uri.add_query_params
      (Uri.of_string
         ((Aws.Util.of_option_exn (Endpoints.url_of service region)) ^
            (((("/" ^ req.GetObjectRetentionRequest.bucket) ^ "/") ^
                req.GetObjectRetentionRequest.key)
               ^ "?retention")))
      (Util.drop_empty
         (Uri.query_of_encoded
            (Query.render (GetObjectRetentionRequest.to_query req)))) in
  (`GET, uri, (Headers.render (GetObjectRetentionRequest.to_headers req)),
    "")
let of_http headers
  ...
  match let ((`String body) :
        ...
        try
          ...
          let resp =
            ...
            | _ ->
                raise
                  (Failure "Could not find well formed ObjectLockRetention.") in
          try
            let open Error in
              Util.or_error (Util.option_bind resp ObjectLockRetention.parse)
                (BadResponse
                   {
                     body;
                     message =
                       "Could not find well formed ObjectLockRetention."
                   })
          with
                  ...
                  (BadResponse
                     {
                       body;
                       message =
                         ("Error parsing ObjectLockRetention - missing field in body or children: "
                            ^ msg)
                     })
        with
        | Failure msg ->
  ...
  with
  | `Ok payload -> `Ok (GetObjectRetentionOutput.of_headers headers payload)
  | `Error err -> `Error err
