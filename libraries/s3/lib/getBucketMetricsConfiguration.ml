    ...
    Uri.add_query_params
      (Uri.of_string
         ((Aws.Util.of_option_exn (Endpoints.url_of service region)) ^
            (("/" ^ req.GetBucketMetricsConfigurationRequest.bucket) ^
               "?metrics")))
      (Util.drop_empty
         (Uri.query_of_encoded
            (Query.render (GetBucketMetricsConfigurationRequest.to_query req)))) in
  (`GET, uri,
    (Headers.render (GetBucketMetricsConfigurationRequest.to_headers req)),
    "")
let of_http headers
  ...
  match let ((`String body) :
        ...
        try
          ...
          let resp =
            ...
            | _ ->
                raise
                  (Failure "Could not find well formed MetricsConfiguration.") in
          try
            let open Error in
              Util.or_error
                (Util.option_bind resp MetricsConfiguration.parse)
                (BadResponse
                   {
                     body;
                     message =
                       "Could not find well formed MetricsConfiguration."
                   })
          with
                  ...
                  (BadResponse
                     {
                       body;
                       message =
                         ("Error parsing MetricsConfiguration - missing field in body or children: "
                            ^ msg)
                     })
        with
        | Failure msg ->
  ...
  with
  | `Ok payload ->
      `Ok (GetBucketMetricsConfigurationOutput.of_headers headers payload)
  | `Error err -> `Error err
