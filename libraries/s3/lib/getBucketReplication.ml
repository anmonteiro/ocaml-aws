...
let of_http headers
        ...
        try
          ...
          let resp =
            ...
            | _ ->
                raise
                  (Failure
                     "Could not find well formed ReplicationConfiguration.") in
          try
            let open Error in
              Util.or_error
                (Util.option_bind resp ReplicationConfiguration.parse)
                (BadResponse
                   {
                     body;
                     message =
                       "Could not find well formed ReplicationConfiguration."
                   })
          with
                  ...
                  (BadResponse
                     {
                       body;
                       message =
                         ("Error parsing ReplicationConfiguration - missing field in body or children: "
                            ^ msg)
                     })
        with
        | Failure msg ->
  ...
  with
  | `Ok payload ->
      `Ok (GetBucketReplicationOutput.of_headers headers payload)
  | `Error err -> `Error err
