    ...
    Uri.add_query_params
      (Uri.of_string
         ((Aws.Util.of_option_exn (Endpoints.url_of service region)) ^
            (("/" ^ req.GetPublicAccessBlockRequest.bucket) ^
               "?publicAccessBlock")))
      (Util.drop_empty
         (Uri.query_of_encoded
            (Query.render (GetPublicAccessBlockRequest.to_query req)))) in
  (`GET, uri, (Headers.render (GetPublicAccessBlockRequest.to_headers req)),
    "")
let of_http headers
        ...
        try
          ...
          let resp =
            ...
            | _ ->
                raise
                  (Failure
                     "Could not find well formed PublicAccessBlockConfiguration.") in
          try
            let open Error in
              Util.or_error
                (Util.option_bind resp PublicAccessBlockConfiguration.parse)
                (BadResponse
                   {
                     body;
                     message =
                       "Could not find well formed PublicAccessBlockConfiguration."
                   })
          with
                  ...
                  (BadResponse
                     {
                       body;
                       message =
                         ("Error parsing PublicAccessBlockConfiguration - missing field in body or children: "
                            ^ msg)
                     })
        with
        | Failure msg ->
  ...
  with
  | `Ok payload ->
      `Ok (GetPublicAccessBlockOutput.of_headers headers payload)
  | `Error err -> `Error err
